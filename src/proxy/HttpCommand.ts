...
class HttpCommand {
	...
	public getGameConfig(siteId:number = 4,gameId:number = 1):void
	{
        
        let curURL:string = `http://${GlobalData.gameDomainURL}/index.php?m=content&c=litAppClock&a=index&siteId=${GlobalData.gameDomainURL}&litAppId=${gameId}&requstID=${HttpCommand.GameConfigID}`;
        this.postRequset(curURL);
    }
    public reportResult(mainURL:string,userId:string,gameId:number,score:number,bonus:number = 0):void
	{
        // alert("userId:"+userId)
		...
		let curURL:string = `http://${GlobalData.gameDomainURL}/index.php?m=content&c=acceptResult&a=add&domain=${mainURL}&userId=${userId}&appId=${gameId}&score=${score}&bonus=${bonus}&requstID=${HttpCommand.reportResultID}`;
        this.postRequset(curURL);
    }
	//排序方式（1本地排行，2全国排行，3预留）
    public getRankList(mainURL:string,gameId:number,sort:number = 0):void
	{
        let curURL:string =
            `http://${GlobalData.gameDomainURL}/index.php?m=content&c=userRankingList&a=showList&domain=${mainURL}&appId=${gameId}&sort=${sort}&requstID=${HttpCommand.rankInfoID}`;
        this.postRequset(curURL);
	}
    ...
    public getUserRank(useID:string):void
    {
        ...
        let curURL:string =
            `http://${GlobalData.gameDomainURL}/index.php?m=content&c=currentUserNumber&a=showIndex&userId=${useID}&gameId=${GlobalData.gameId}&requstID=${HttpCommand.userRank}`;
        this.postRequset(curURL);
        // alert(curURL);
    }
		...
		if(parseInt(result["StatusCode"])>0)//成功
		{
			...
			switch (requsetID)
			{
                case HttpCommand.GameConfigID:
                    ...
                    EventManager.dispatchEvent(new CommonEvent(CommonEvent.GET_INFO_SUCESS));
                    break;
                case HttpCommand.rankInfoID:
                    if(objs.length<=0)
                    {
                        EventManager.dispatchEvent(new CommonEvent(CommonEvent.GET_RANK_SUCESS,0));
                        return;
                    }
                    ...
                    EventManager.dispatchEvent(new CommonEvent(CommonEvent.GET_RANK_SUCESS,parseInt(objs[0]["sort"])));
                    break;
                case HttpCommand.reportResultID:
                    console.log("上报成功！！");
                    EventManager.dispatchEvent(new CommonEvent(CommonEvent.UP_SOCRE_SUCESS));
                    break;
                case HttpCommand.userRank:
                    // alert("userRank:"+objs);
                    if(objs)
                    {
                        EventManager.dispatchEvent(new CommonEvent(CommonEvent.GET_USERRANK_SUCESS,objs));
                    }
                    else
                    {
                        // alert("数据为空！");
